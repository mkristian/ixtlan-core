...
def create_rails_application(template, tests = nil)
  ...
  args = "rails new #{app_directory} -f -m templates/#{template} -- -DrailsVersion=#{rails_version} -Dgem.home=#{ENV['GEM_HOME']} -Dgem.path=#{ENV['GEM_PATH']}"# -Dplugin.version=0.26.0-SNAPSHOT"
